...
@section('title','Edit Kategori Pengeluaran') {{-- Judul halaman pada title tag. --}}

@section('content') {{-- Mulai section konten utama. --}}

<h5 class="fw-bold mb-4">Edit Kategori Pengeluaran</h5> {{-- Judul kecil halaman. --}}

<div class="card-artakula p-4 col-md-6"> {{-- Card wrapper form. --}}

    {{-- Form edit kategori --}}
    <form method="POST"
          action="{{ route('kategori_pengeluaran.update', $kategori->id) }}"> {{-- Form update kategori. --}}
        @csrf {{-- Token CSRF. --}}
        ...
        <div class="mb-3"> {{-- Grup input periode awal. --}}
            ...
            <input type="date"
                   name="periode_awal"
                   class="form-control"
                   value="{{ old('periode_awal', $kategori->periode_awal) }}"> {{-- Input periode awal. --}}
        ...
        <div class="mb-3"> {{-- Grup input periode akhir. --}}
            ...
            <input type="date"
                   name="periode_akhir"
                   class="form-control"
                   value="{{ old('periode_akhir', $kategori->periode_akhir) }}"> {{-- Input periode akhir. --}}
